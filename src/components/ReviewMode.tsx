...
export default function ReviewMode({ sciences }: { sciences: Science[] }) {
  ...
  const [relatedFawaid, setRelatedFawaid] = useState<Fawaid[]>([]);
  ...
  useEffect(() => {
    if (showContent && currentNote) {
      const fetchRelated = async () => {
        setLoadingRelated(true);
        try {
          const res = await fetch(`/api/search/semantic?search=${encodeURIComponent(currentNote.title || currentNote.content)}`);
          if (res.ok) {
            const data = await res.json();
            setRelatedFawaid(data.filter((f: Fawaid) => f.id !== currentNote.id).slice(0, 3));
          }
        } catch (e) {
          console.error(e);
        } finally {
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white dark:bg-zinc-900 rounded-[40px] p-12 border border-[#E5E5E0] dark:border-zinc-800 shadow-xl shadow-[#5A5A40]/5 dark:shadow-none relative overflow-hidden min-h-[400px] flex flex-col">
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-8">
              <h4 className="text-3xl aref-ruqaa-regular text-[#1A1A1A] dark:text-white leading-tight" dir="rtl">
