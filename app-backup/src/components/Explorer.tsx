...
export default function Explorer({ sciences, onUpdate }: { sciences: Science[], onUpdate?: () => void | Promise<void> }) {
  ...
  const performCopy = (fawaid: Fawaid | Fawaid[]) => {
  ...
  const handleDeleteBook = async (id: number) => {
    if (!confirm(t('Delete Book Confirmation'))) return;
    try {
  ...
  const handleAddFaidah = () => {
    if (!selectedScience || !selectedBook) {
      alert(t('Select Book First'));
      return;
    }
  ...
  const handleDeleteScience = async (id: number) => {
    if (!confirm(t('Delete Science Confirmation'))) return;
    try {
  ...
  const submitEditScience = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(getScienceErrorMessage(error?.code) || error?.error || t('Error'));
        return;
      }
          ...
          <div>
            ...
            <nav className="flex items-center gap-2 text-sm text-[#8E8E8E] dark:text-gray-400 mt-1">
              ...
              {selectedBook && (
                <>
                  <ChevronRight className="w-3 h-3" />
                  <span className="text-[#5A5A40] dark:text-zinc-300 font-medium">{selectedBook.title}</span>
                </>
              )}
              {selectedSharh && selectedSharh.id !== -1 && (
                <>
                  <ChevronRight className="w-3 h-3" />
                  <span className="text-[#5A5A40] dark:text-zinc-300 font-medium">{selectedSharh.title}</span>
                </>
              )}
            </nav>
          </div>
                ...
                return (
                  <div className="space-y-4 mb-6">
                    <input 
                      ...
                      className="font-serif font-bold text-lg w-full border-b border-[#E5E5E0] dark:border-zinc-700 pb-2 focus:outline-none focus:border-[#5A5A40] bg-transparent dark:text-white"
                      placeholder={t('Title Placeholder')}
                    />
                    <textarea 
                      value={note.content}
                      onChange={e => setItems(items.map(i => i.id === editingId ? {...i, content: e.target.value} : i))}
                      className="w-full h-32 resize-none text-sm font-serif leading-relaxed focus:outline-none bg-transparent dark:text-gray-300"
                    ...
                    <textarea 
                      ...
                      className="w-full h-16 resize-none text-xs font-serif leading-relaxed focus:outline-none bg-[#F5F5F0] dark:bg-zinc-800 dark:text-gray-300 p-2 rounded-md"
          ...
          >
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h3 className="text-2xl font-serif font-bold dark:text-white">{expandedNote.title || t('Untitled Note')}</h3>
                  <div className="flex items-center gap-2 mt-2 text-sm text-[#8E8E8E] dark:text-gray-400">
                ...
                <button 
                  onClick={() => setExpandedNote(null)}
                  className="p-2 hover:bg-[#F5F5F0] dark:hover:bg-zinc-800 rounded-full transition-all dark:text-white"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="prose dark:prose-invert max-w-none">
                <p className="text-lg font-serif leading-relaxed whitespace-pre-wrap dark:text-gray-300" dir="auto">
                  {expandedNote.content}
                </p>
                {expandedNote.extra_notes && (
                  <div className="mt-6 p-4 bg-emerald-50/50 dark:bg-emerald-900/10 rounded-xl border border-emerald-100 dark:border-emerald-900/30">
                    <h4 className="text-sm font-bold text-emerald-600 dark:text-emerald-500 mb-2 uppercase tracking-wider">{t('Extra Notes')}</h4>
                    <p className="text-md font-serif leading-relaxed whitespace-pre-wrap dark:text-gray-300" dir="auto">
                      {expandedNote.extra_notes}
                    </p>
                  </div>
                )}
...
function BookCard({ 
  ...
  return (
    ...
    >
      ...
      <h3 className="text-lg font-serif font-bold mb-2 flex-1 dark:text-white">{book.title}</h3>
...
function SharhCard({ sharh, onClick, onDelete, onRename, t, isGeneral }: { sharh: Sharh, onClick: () => void, onDelete: () => void, onRename: () => void, t: any, isGeneral?: boolean }) {
  return (
    ...
    >
      ...
      <h3 className="text-lg font-serif font-bold mb-2 flex-1 dark:text-white">{sharh.title}</h3>
...
function FawaidCard({ 
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-[#E5E5E0] dark:border-zinc-800 shadow-sm flex flex-col h-full group hover:border-[#5A5A40] dark:hover:border-zinc-600 transition-all">
      ...
      <h3 className="text-lg font-serif font-bold mb-3 dark:text-white text-right">{fawaid.title || t('Untitled Note')}</h3>
      
      <p className="text-sm text-[#4A4A4A] dark:text-gray-300 font-serif leading-relaxed mb-6 flex-1 line-clamp-4 italic text-right" dir="auto">
        "{fawaid.content}"
      </p>
